resource "azurerm_network_interface" "nic" {
  count    = var.linux-server.nic-count
  ...
  ip_configuration {
    name = "IPv4Config"
    private_ip_address            = var.linux-server.private-ip-address
    private_ip_address_allocation = var.linux-server.private-ip-alloc
    public_ip_address_id          = var.linux-server.public-ip-id
    subnet_id                     = var.linux-server.subnet-id
  }
